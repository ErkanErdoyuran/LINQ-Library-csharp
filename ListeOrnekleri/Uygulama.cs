...
namespace ListeOrnekleri
{
    class Uygulama
    {
        public void Calistir()
        {
            ...
            yeniListe = ogrenciler.OrderBy(a => a.Not).Take(5).ToList();

            yeniListe = ogrenciler.Where(item=> item.Alan==ALAN.EsitAgirlik&&item.Subesi==SUBE.C).Take(3).ToList();


            yeniListe = ogrenciler.Where(a => a.No == 15).ToList();

            Ogrenci ogr = ogrenciler.Where(a => a.No == 15).FirstOrDefault();




            //yeniListe = ogrenciler.Where(a => a.Alan == ALAN.Sayisal).OrderByDescending(a => a.Not).Take(3).ToList();

            //int toplam = yeniListe.Sum(a => a.Yas);

            yeniListe = ogrenciler.Where(a => a.Alan == ALAN.Sozel).ToList();

            int a = yeniListe.Max(a => a.Not);
